...
from booki.utils.log import logBookHistory, logChapterHistory,  logSectionHistory, printStack
from booki.exportadorCnxml.exportador import convertir
from booki.exportadorCnxml.rhaptos_test import zip_and_updload_files
...
def getHoldChapters(book_version):
    ...
    return [(ch.id, ch.title, ch.url_title, 1, ch.status.id, ch.parent) for ch in book_version.getHoldChapters()]


def getSubChapters(book_version):
    """
    Function returns list of hold chapters. Elements of list are tuples with structure - (chapter_id, chapter_title, chapter_url_title, 1, chapter_status_id).
    ...
    @param book_version: Book version object
    @rtype: C{list}
    @return: Returns list with subchapters
    """

    return [(ch.id, ch.title, ch.url_title, 1, ch.status.id, ch.parent) for ch in book_version.getSubChapters()]
...
def remote_init_editor(request, message, bookid, version):
    ...
    subChapters = getSubChapters(book_version)
    ## get users
    def _getUserName(a):
...
def remote_change_status(request, message, bookid, version):
    ...
    msgid = "user_changed_section_status"
    if chapter.typeof>0:
         msgid = "user_changed_chapter_status"

    if chapter.parent:
         msgid = "user_changed_subchapter_status"
...
def remote_chapter_export(request, message, bookid, version):
    print os.getcwd()
    path_actual = os.getcwd()

    # LEO LOS CAMPOS DE LA TABLA editor_chapter DE BOOKTYPE
    chapter = models.Chapter.objects.get(id=int(message["chapterID"]))
    title_chapter = chapter.title
    content_html = chapter.content

    # REALIZAR LA CONVERSION DEL CAPITULO EN HTML A CNXML
    convertir(content_html)

    # COMPRIMIR EL DIR DE SALIDA Y ENVIAR A RHAPTOS
    server = 'http://cnx.org/Members/proyectolatin/sword'
    dir_file = 'cnxml_output'
    metadata = dict()
    metadata['title'] = title_chapter
    metadata['author'] = "Yo"
    metadata['summary'] = ""
    # Usuario y claves eliminados a continuación
    metadata['username'] = ""
    metadata['password'] = ""
    metadata['subject'] = "Science and Technology"
    metadata['language'] = "es"
    zip_and_updload_files(server, dir_file, metadata)
...
def remote_chapter_save(request, message, bookid, version):
    ...
    book_version = book.getVersion(version)
    chapter = models.Chapter.objects.get(id=int(message["chapterID"]))
    msgid = "user_saved_section"

    if chapter.typeof>0:
         msgid = "user_saved_chapter"

    if chapter.parent:
         msgid = "user_saved_subchapter"
    ...
    chapter.raw_content = message["raw_content"];
...
def remote_chapter_rename(request, message, bookid, version):
    ...
    msgid = "user_renamed_section"

    if chapter.typeof>0:
         msgid = "user_renamed_chapter"

    if chapter.parent:
         msgid = "user_renamed_subchapter"
...
def remote_get_section(request, message, bookid, version):
    """
    This is called when you fire up WYSWYG editor or Chapter viewer. It sends back basic chapter information.
    ...
    section = models.Section.objects.get(id=int(message["sectionID"]))
    res["title"] = section.title
    res["content"] = section.content
    res["current_revision"] = section.revision

    if message.get("revisions", False):
        res["revisions"] = [x.revision for x in models.SectionHistory.objects.filter(section=section).order_by("revision")]


    if message.get("revision", section.revision) != section.revision:
        ch = models.SectionHistory.objects.get(section=section, revision=message.get("revision"))
        res["content"] = ch.content
  ...
  #  import time
    ...
    # set the initial timer for editor
   # sputnik.set("booki:%s:locks:%s:%s" % (bookid, message["sectionID"], request.user.username), time.time())

    #sputnik.addMessageToChannel(request, "/booki/book/%s/%s/" % (bookid, version),
                              #  {"command": "chapter_status",
                              #   "chapterID": message["sectionID"],
                              #   "status": "edit",
                              #   "username": request.user.username})
...
def remote_create_chapter(request, message, bookid, version):
    ...
    parent = None
    pos = 0
    msgid = "user_new_chapter"
    if message["parent"]:
        msgid = "user_new_subchapter"
        parent = message["parent"]
        #calculate position of new item
        subchaplist = models.Chapter.objects.filter(book=book, version=book_version, parent=parent).order_by("-sc_position")
        if len(list(subchaplist)) > 0:
                 pos = subchaplist[0].sc_position
    ...
    chapter = models.Chapter(book = book,
                             ...
                             content = content,
                             created = datetime.datetime.now(),
                             modified = datetime.datetime.now(), parent = parent, typeof = 1, sc_position=pos)
    ...
    result = (chapter.id, chapter.title, chapter.url_title, 1, s.id,chapter.parent)
...
def remote_create_section_old(request, message, bookid, version):
    """
...
def remote_create_section(request, message, bookid, version):
    ...
    section = models.Chapter(book = book,
                             version = book_version,
                             url_title = url_title,
                             title = message["chapter"],
                             status = s,
                             content = content,
                             created = datetime.datetime.now(),
                             modified = datetime.datetime.now(),typeof = 0)
    ...
    try:
        ...
        # this should be solved in better way
        # should have createSection in booki.utils.book module
        ...
        history = logChapterHistory(chapter = section,
                                    content = content,
                                    user = request.user,
                                    comment = message.get("comment", ""),
    ...
    result = (section.id, section.title, section.url_title, 0, s.id,section.parent)
    ...
    #result = ("s%s" % tc.id, tc.name, None, tc.typeof)



    sputnik.addMessageToChannel(request, "/booki/book/%s/%s/" %  (bookid, version), {"command": "chapter_create", "chapter": result, "typeof": tc.typeof},myself = True)


    return {"created": True}


def remote_create_section_s(request, message, bookid, version):
    """
    Creates new chapters. New chapter is always added at the end of the TOC. Creates empty chapter content only with chapter title as heading.
    ...
    ch = models.Section.objects.filter(book=book, version=book_version, url_title=url_title)
    ...
    section = models.Section(book = book,
                             version = book_version,
                             url_title = url_title,
                             title = message["chapter"],
                             status = s,
    ...
    try:
        ...
        history = logSectionHistory(section = section,
                                    content = content,
                                    user = request.user,
                                    comment = message.get("comment", ""),
                                    revision = section.revision)
...
def remote_get_chapter_content(request, message, bookid, version):
    """
    Returns Chapter content in etherpad
    ...
    try:
        chaptercontent = models.Store.objects.using('etherpad').get(key=message["chapterID"])
        return {"content": chaptercontent.value}
...
def remote_chapter_delete(request, message, bookid, version):
    ...
    Creates Book history record. Sends notification to chat. Sends "chapter_status" message to the channel. Sends "chapter_delete" message to the channel.
...
def remote_create_eventlog_entry(request, message, bookid, version):
    """
    Creates new section. Sections is added at the end of the TOC.
   ...
   # access_ip character varying(15) NOT NULL,
   ...
   # event_timestamp timestamp with time zone NOT NULL
    ...
    eventlog = models.EventsLog(username = request.user.username,
                       #access_ip = message["access_ip"],
                       access_ip = ip,
...
def remote_change_chapter_position(request, message, bookid, version):

    chapter1 = models.Chapter.objects.get(id=int(message["chapter1"]))

    chapter2 = models.Chapter.objects.get(id=int(message["chapter2"]))

    new_position2 = chapter1.sc_position
    new_position1 = chapter2.sc_position

    chapter2.sc_position = new_position2
    chapter1.sc_position = new_position1
...
###
 # MODIFICATION UCSP
 # Funciones para la creacion e integracion del sistema de discussion

def remote_create_discussion(request, message, bookid, version):
    """
    Funcion para crear el tema de discucion en el modelo
    """
    import datetime
    book = models.Book.objects.get(id=bookid)
    user = request.user
    ...
    guid_array = discussions_mysql.Get_Last_Id('elgg_objects_entity')
    guid = guid_array[0] + 1
    ...
    discussion.append({"title": discussiontheme.title,
		 ...
		 "created": discussiontheme.created.strftime("%b.%d, %Y, %H:%M")})
...
def remote_edit_discussion(request, message, bookid, version):
    """
    Funcion para editar el tema de discucion en el modelo
    """
    import datetime
...
def remote_get_discussion_list(request, message, bookid, version):
    """
    Funcion para obtener la lista de discusiones
    """
    book = models.Book.objects.get(id=bookid)
	...
	if discussion.access == 'public' or discussion.access == 'groupmembers' and is_member or discussion.access == 'private' and discussion.user == request.user:
	    discussions.append({"title": discussion.title,
				  ...
				  "created": discussion.created.strftime("%b.%d, %Y, %H:%M")})
    except:
      return {"get":False}
      transaction.rollback()
    else:
...
def remote_get_discussion_list_by_label(request, message, bookid, version):
    """
    Funcion para obtener la lista de discusiones de acuerdo a un label
    """
    book = models.Book.objects.get(id=bookid)
    discussion_list = models.DiscussionTheme.objects.filter(book = book).order_by('created')
    group = book.group
...
def remote_get_comment_list(request, message, bookid, version):
	...
	# Veo si el usuario puede eliminar el comentario -- solo el dueno del grupo de discusion o dueno del comentario
	can_remove = 0
	if request.user == comment.user or request.user == discussion.user:
	  can_remove = 1
	# Saco el tiempo en formato unix
	created = comment.created
        unix_created = time.mktime(created.timetuple())*1e3 + created.microsecond/1e3

	# Veo si el comentario ha sido votado por el usuario
	comment_is_like = 0
	...
	if comment_is_like == 1:
	      comments.append({"user": comment.user.username,
				...
				"like_request_user": request.user.username,
				"like_is_like": 1,
				"like_next_value":'Ya No Me Gusta',
...
def remote_get_discussion_detail(request, message, bookid, version):
    """
    Funcion para obtener el detalle de la discusion
    """
    discussion_title = message['title']
    try:
      discussion = models.DiscussionTheme.objects.get(title = discussion_title)
...
def remote_add_comment_to_theme(request, message, bookid, version):
    """
    Funcion para anadir un comentario a un tema de discusion
    """
    import datetime
    ...
    comm.append({"comment": commenttheme.comment,
		 ...
		 "created": commenttheme.created.strftime("%b.%d, %Y, %H:%M"),
...
def remote_update_like_to_comment(request, message, bookid, version):
    """
    Funcion para actualizar el like de un usuario respecto a un comentario dentro de un tema de discusion
    """
    import time
    import datetime
...
def remote_delete_all_likes_from_comment(request, message, bookid, version):
    """
    Funcion para eliminar todos los likes de un comentario
    """
...
def remote_delete_comment_from_theme(request, message, bookid, version):
    """
    Funcion para eliminar un comentario de un tema de discusion
    """
    try:
...
def remote_delete_themediscussion(request, message, bookid, version):
    """
    Funcion para eliminar un tema de discusion
    """
    try:
      theme = models.DiscussionTheme.objects.get(title = message['title'])
    except:
