...
<!-- MODIFIQUE PARA QUE FUNCIONE, ADEMAS ME BASE EN EL CODIGO HTML DE LA TABLA QUE GENERA ETHERPAD, SIN THEAD, SIN TFOOT (VALERIA GERLING)-->
       ...
       <xsl:for-each select="xh:tr">
         <row>
           <xsl:for-each select="xh:td">
             <entry> 
              <!-- MODIFIQUE EL SELECT, EN VEZ DE DECIR SELECT = "*", PUSE SELECT = ".", PORQUE NO FUNCIONABA PARA VALORES HOJAS DE TD (VALERIA GERLING) -->
...
<!-- AGREGADO POR VALERIA GERLING -->
<!-- videos -->
<xsl:template match="xh:iframe" mode="pass6">
  <cnxtra:video>
    <xsl:copy-of select="@src"/>
  </cnxtra:video>
</xsl:template>

<xsl:template match="xh:script" mode="pass6">
  <pepe>
    hola
  </pepe>
</xsl:template>
