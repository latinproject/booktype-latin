...
class BookVersion(models.Model):
    ...
    def getSubChapters(self):
        return Chapter.objects.raw('SELECT * FROM editor_chapter where book_id=%s and version_id=%s and parent >0 ORDER BY parent, sc_position', (self.book.id, self.id))
...
#    comment = models.CharField(_('comment'), max_length=2500, blank=True)
    ...
    raw_content = models.TextField(_('raw_content'))

    typeof = models.SmallIntegerField(_('typeof'), choices=TYPEOF_CHOICES)
    sc_position = models.SmallIntegerField(_('sc_position'),  null=True)
...
class Store(models.Model):
    ...
    key = models.CharField(_('key'),max_length=300, primary_key=True)
...
class EventsLog(models.Model):
    ...
    access_ip = models.CharField(_('access_ip'),max_length=15)
    event_name = models.CharField(_('event_name'),max_length=50)
    event_timestamp = models.DateTimeField(_('event_timestamp'), auto_now=False, default=datetime.datetime.now)
    book = models.ForeignKey(Book, null=True, verbose_name=_("book"))
    comment = models.TextField(_('comment'), null=True)
    class Meta:
        verbose_name = _('EventsLog')
        verbose_name_plural = _('EventsLog')


###
 # MODIFICATION UCSP
 # - Modelos para la creacion de los temas de discusion
 #

# Modelo del Tema de Discusion
class DiscussionTheme(models.Model):
    ...
    labels = models.TextField(_('labels'))
    state = models.CharField(_('state'), max_length=20)
    access = models.CharField(_('access'), max_length=50)
    created = models.DateTimeField(_('created'), null=False, auto_now=False, default=datetime.datetime.now)

    def get_created_string(self):
       time = created.strftime("%b.%d, %Y, %H:%M")
       return self.time
...
class DiscussionComment(models.Model):
    discussiontheme = models.ForeignKey(DiscussionTheme, verbose_name=_("discussiontheme"))
    user = models.ForeignKey(auth_models.User, verbose_name=_("user"))

    comment = models.TextField(_('comment'))
    created = models.DateTimeField(_('created'), null=False, auto_now=False, default=datetime.datetime.now)
    nro_likes = models.IntegerField(_('nro likes'), default=0)

    def __unicode__(self):
        return self.comment

# Modelo del Like para los comentarios
class DiscussionCommentLike(models.Model):
    discussioncomment = models.ForeignKey(DiscussionComment, verbose_name=_("discussioncomment"))
    user = models.ForeignKey(auth_models.User, verbose_name=_("user"))
