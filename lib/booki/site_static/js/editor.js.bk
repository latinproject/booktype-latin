...
$(function() {
    ...
    jQuery.booki.editor = function() {
        ...
        var coverFilter = "all";
        var subChapters = null;           //save all subchapters by parent
                ...
                $.each(this.items, function(i, v) {
                    if(v.isChapter){
                        if (v.parent==null || v.parent == 0){
                            ...
                            //check if the chapter has subchapters
                            listsubchapters = [];
                            idx = 0;

                            for(var n = 0; n < subChapters.length; n++) {
                                elem = subChapters[n];
                                if (subChapters[n][5] == v.id){
                                    listsubchapters.push( createChapter({id: elem[0], title: elem[1], isChapter: elem[3] == 1, status: elem[4], url_title: elem[2], parent: elem[5]}));
                                    idx++;
                                }
                            }
                            if (idx>0){
                                makeChapterCompleteLine(v.id, v.title, getStatusDescription(v.status),listsubchapters).appendTo($this.containerName);
                            }
                            else
                        ...
                        makeSectionLine(v.id, v.title, getStatusDescription(v.status)).appendTo($this.containerName);
                    }
...
//        $(".extra").html("");
        ...
        $.extend(HistoryPanel.prototype, {
            ...
            'setHistoryView': function() {
                ...
                function _drawItems(data) {
			      ...
			      $.booki.ui.fillTemplate(en, {"entryFilename": '"'+fname+'"',
                                "entryUser": entry.user,
                                "entryModified": entry.modified});
         ...
         */
        function makeSectionLine(chapterID, name, status) {
            ...
            line.find('.deletechapter').click(function() {
                $.booki.editor.deleteChapter(chapterID);
            });
            var statusName = $(".bookStatuses.template #"+status.replace(/\s/g, "_")).html();
            line.find('.statusName').first().html(escapeJS(statusName));
            line.dblclick(function() {
                ...
                function _doRename() {
                    ...
                    $("#item_"+chapterID).replaceWith(makeSectionLine(chapterID, newName, status));
                    toc.refreshLocks();
                    holdChapters.refreshLocks();
                ...
                $("FORM", $this).append($('<a href="#">'+$.booki._('CANCEL', 'CANCEL')+'</a>').click(function() {
                    ...
                    $("#item_"+chapterID).replaceWith(makeSubChapterLine(chapterID, ch.title, getStatusDescription(ch.status)));
                    return false;
                }));
        ...
        function makeChapterLine(chapterID, name, status) {
            ...
            line.find('.subchapterLink').click(function() {
                $.booki.editor.createSubChapter(chapterID);
            });
            line.find('.editLink').click(function() {
                $.booki.editor.editChapter(chapterID);
            });
            ...
            line.find('.exportRhaptos').click(function() {
            $.booki.editor.exportRChapter(chapterID);
            });
        ...
        /*
         Creates HTML code for the Chapter line in TOC. It also binds different events.
         */
        function makeSubChapterLine(chapterID, name, status) {

            var line = $.booki.ui.getTemplate('subChapterLine');

            line.attr('id', 'item_'+chapterID);
            $.booki.ui.fillTemplate(line, {'title': escapeJS(name)});

            line.find('.editsLink').click(function() {
                $.booki.editor.editChapter(chapterID);
            });
            line.find('.viewsLink').click(function() {
                $.booki.editor.viewChapter(chapterID);
            });
            line.find('.lockUser').click(function() {
                $.booki.editor.unlockChapter(chapterID);
            ...
            line.find('.statussLink').click(function() {
                $.booki.editor.editStatusForChapter(chapterID);
            });
            line.find('.deletesubchapter').click(function() {
                $.booki.editor.deleteSubChapter(chapterID);
            });
            line.find('.moveUp').click(function() {
                $.booki.editor.moveUpSubchapter(chapterID);
            });
            line.find('.moveDown').click(function() {
                $.booki.editor.moveDownSubchapter(chapterID);
            });
            line.find('.exportRhaptos').click(function() {
            $.booki.editor.exportRChapter(chapterID);
            });
            ...
            line.dblclick(function() {
                ...
                function _doRename() {
                    ...
                    $("#item_"+chapterID).replaceWith(makeSubChapterLine(chapterID, newName, status));
                    toc.refreshLocks();
                    holdChapters.refreshLocks();
        ...
        /*
         Creates HTML code for the Chapter line in TOC. It also binds different events.
         */
        function makeChapterCompleteLine(chapterID, name, status, subchapters) {


            //jQuery.booki.sendToCurrentBook({"command": "get_subchapters","chapterID":chapterID},function(data) {

            /*  var att_html = $("#subchaptercontainer");
             var s = $.booki.ui.getTemplate('subchaptersList');
             for(var i = 0; i < subchapters.length; i++) {
             s.append('<li>'+subchapters[i].title+'</li>');
             }


             att_html.empty().append(s);*/

            var line = $.booki.ui.getTemplate('completechapterLine');
            ...
            var sublist  = line.find('.subchapterslist');
            var sublist  =
                sublist.empty();
            for(var i = 0; i < subchapters.length; i++) {
                v = subchapters[i];
                makeSubChapterLine(v.id, v.title, getStatusDescription(v.status)).appendTo(sublist);
            }
            ...
            line.dblclick(function() {
                ...
                function _doRename() {
                    ...
                    $("#item_"+chapterID).replaceWith(makeChapterCompleteLine(chapterID, newName, status,subchapters));
                    toc.refreshLocks();
                    holdChapters.refreshLocks();
                ...
                $("FORM", $this).append($('<a href="#">'+$.booki._('CANCEL', 'CANCEL')+'</a>').click(function() {
                    ...
                    // this is not god. should get info from toc
                    var ch = getChapter(chapterID);
                    $("#item_"+chapterID).replaceWith(makeChapterCompleteLine(chapterID, ch.title, getStatusDescription(ch.status),subchapters));
                    return false;
                }));
        ...
        return {
            ...
            createSubChapter: function(chapterID) {
                var chap = getChapter(chapterID);
                var dial = $.booki.ui.getTemplate('subChapterDialog');
                ...
                dial.find("p").html( $.booki._('label_subchapter', 'Create subchapter for chapter: ') + "<b>" + chap.title +"</b>");
                ...
                dial.dialog({
                        ...
                        width: 400,
                        buttons: [
                            {text: $.booki._('create_subchapter', 'Create subchapter'),
                                click: function() {

                                    var subchapter =dial.find('input').val();

                                    if($.trim(subchapter)=='') return;

                                    $.booki.ui.notify($.booki._('creating_subchapter', 'Creating subchapter'));

                                    $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_chapter", "chapter": subchapter, "parent":chapterID}, function(data) {
                                        if(data.created) {
                                            $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "create_subchapter", "comment": "Subchapter name:"+subchapter}, function(data2) {
                                                $.booki.ui.notify();
                                            });
                                        } else {
                                            $.booki.ui.notify();
                                            if(typeof data.silly_url != 'undefined' && data.silly_url == true)
                                                alert($.booki._('can_not_create_subchapter_illegal', 'This is not a valid chapter name.'));
                                            else
                                                alert($.booki._('can_not_create_subchapter', 'Can not create duplicate chapter name.'));
                                        }
                                        checkForEmptyTOC();
                                    });
                        ...
                        open: function(event,ui) {
                            $("INPUT", $(this)).val($.booki._('enter_new_subchapter', 'Enter new Subchapter title.')).select();
                        },
                    }
                );
            ...
            deleteChapter: function(chapterID) {
                ...
                disp.dialog({modal: true,
                    ...
                    buttons: [
                        ...
                        {text: $.booki._('remove_chapter', 'Delete content'),
                            ...
                            click:   function() {
                                ...
                                $.booki.sendToCurrentBook({"command": "chapter_delete", "chapterID": chapterID}, function() {
                                    $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "delete_content", "comment": "Content ID:"+chapterID}, function(data2) {});
                                    $dialog.dialog('close');
                                });
                            }
                        }
            ...
            moveUpSubchapter: function(chapterID) {
                var liItem = $("#item_"+chapterID);
                if (liItem.prev().is('li'))
                {
                    var previd = liItem.prev().attr('id');
                    var prevchapterid = previd.replace( /^\D+/g, '');

                    $.booki.sendToCurrentBook({"command": "change_chapter_position", "chapter1": chapterID, "chapter2":prevchapterid}, function() {
                        //$.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "change_chapter_position", "chapter1": chapterID, "chapter2":prevchapterid}, function(data) {
            ...
            moveDownSubchapter: function(chapterID) {
                var liItem = $("#item_"+chapterID);
                if (liItem.next().is('li'))
                {
                    var nextid = liItem.next().attr('id');
                    var nextchapterid = nextid.replace( /^\D+/g, '');
                    $.booki.sendToCurrentBook({"command": "change_chapter_position", "chapter1": chapterID, "chapter2":nextchapterid}, function() {
                        liItem.insertAfter(liItem.next());
                    });
                }
            },
            deleteSubChapter: function(chapterID) {
                    ...
                    buttons: [
                        ...
                        {text: $.booki._('remove_chapter', 'Delete content'),
                            ...
                            click:   function() {
                                ...
                                $.booki.sendToCurrentBook({"command": "chapter_delete", "chapterID": chapterID}, function() {
                                    $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "delete_subchapter", "comment": "Content ID:"+chapterID}, function(data2) {});
                                    $dialog.dialog('close');
                                });
                            }
                        }
            ...
            exportRChapter: function(chapterID) {
                alert($.booki._('start_exportation', 'Wait a moment while the export takes place. Thanks.'));
                $.booki.sendToCurrentBook({"command": "chapter_export", "chapterID": chapterID}, function() {
                    alert($.booki._('end_exportation', 'Export completed successfully.'));
                    });
                $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "chapter_export"}, function() {
                                $.booki.ui.notify();
                              });
            },
                                ...
                                click: function() {
                                    ...
                                    $.booki.sendToCurrentBook({"command": "change_status",
                                        ...
                                        function(data) {
                                            ...
                                            $("#item_"+chapterID+" .status").first().html('<a href="javascript:void(0)" onclick="$.booki.editor.editStatusForChapter('+chapterID+')">'+getStatusDescription(chap.status)+'</a>');
                                        });
                                }
                            },
                            {text: $.booki._('cancel', 'Cancel'),
                ...
                $.booki.sendToCurrentBook({"command": "get_chapter", "chapterID": chapterID}, function(data) {
                    ...
                    $("#container").fadeOut("slow", function() {
                        ...
                        $("#editorcontainer").html(ed).css("display", "block").fadeIn("slow", function() {
                            ...
                            var eth_username = $.booki.username;

                            var srcIframe ="http://etherpad.proyectolatin.org/p/chapter_"+chapterID+"?userName=" + eth_username + "&showChat=false";
                            $("#editorcontainer iframe").attr('src', srcIframe);
                            ...
                            $("#editorcontainer A[name=save]").click(function() {
                                ...
                                var keycontent = "pad:chapter_"+chapterID; //agregado por GC
                                var chaptercontent =""; //agregado por GC
                                var content = ""; //agregado por GC
                                var raw_content = ""; //agregado por GC
                                ...
                                if(xmlHttp ){
                                    xmlHttp.onreadystatechange =  function(){
                                        if(xmlHttp  && xmlHttp.readyState == 4){
                                            ...
                                            xmlHttp  = null; //delete your XMLHTTPRequest
                                        }
                                    }
                                    xmlHttp.open("GET","http://etherpad.proyectolatin.org/api/1/getHTML?apikey=IjxXQp7WCynvWiq3EM1rS2WWXeV8rHUN&padID=chapter_"+chapterID,false);
                                    xmlHttp.send(null);
                                }

                                if (contenido!=""){
                                    var parsedJSON = $.parseJSON(contenido );
                                    if (parsedJSON.data.html){
                                        content = parsedJSON.data.html;
                                    }

                                }


                                $.booki.sendToCurrentBook({"command": "get_chapter_content", "chapterID": keycontent}, function(data2) {
                                    chaptercontent = data2.content;
                                    raw_content = chaptercontent;
                                    ...
                                    $.booki.sendToCurrentBook({"command": "chapter_save",
                                        ...
                                        function() {
                                            delete chapterLocks[chapterID];
                                            $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "save_content", "comment": "Content ID:"+chapterID}, function(data2) {
                                                $.booki.ui.notify();
                                            });
                                            closeEditor();
                                            /*
                    ...
                    buttons: [
                        { text: $.booki._('create_chapter', 'Create chapter'),
                            click: function() {
                                ...
                                $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_chapter", "chapter": $("#newchapter INPUT").val(), "parent":null}, function(data) {
                                    if(data.created) {
                                        $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "create_chapter", "comment": "Chapter name:"+$("#newchapter INPUT").val()}, function(data2) {
                                            $.booki.ui.notify();
                                        });
                                    } else {
                                        $.booki.ui.notify();
                        ...
                        { text: $.booki._('create_section', 'Create section'),
                            click: function() {
                                ...
                                $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_section", "chapter": $("#newsection INPUT").val()}, function(data) {
                                    if(data.created) {
                                        //record in log
                                        $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "create_section", "comment": "Section name:"+$("#newsection INPUT").val()}, function(data2) {
                                            $.booki.ui.notify();
                                        });
        ...
        /*$(".buttonlikecomment").click(function(){
            var comment = $(this).attr("val");
            var func = $(this).attr("func");
            addLiketoComment(theme,comment, func);
        });*/
                ...
                jQuery.booki.sendToCurrentBook({"command": "init_editor"},
                    ...
                    function(data) {
                        ...
                        $.each(data.chapters, function(i, elem) {
                            toc.addItem(createChapter({id: elem[0], title: elem[1], isChapter: elem[3] == 1, status: elem[4], url_title: elem[2], parent: elem[5]}));
                        });

                        $.each(data.hold, function(i, elem) {
                            holdChapters.addItem(createChapter({id: elem[0], title: elem[1], isChapter: elem[3] == 1, status: elem[4], url_title: elem[2], parent: elem[5]}));
                        });
             ...
             */
            initEditor: function() {
                $.booki.sendToChannel("/booki/book/"+$.booki.currentBookID+"/"+$.booki.currentVersion+"/", {"command": "create_eventlog_entry", "event_name": "edit_book", "comment": ""}, function(data2) {});
                    ...
                    var funcs = {
                        ...
                        "chapter_create": function() {
                            ...
                            if(message.chapter[3] == 1) {
                                //parent en pos 5
                                if (message.chapter[5]>0){
                                    listsubchapters = [];
                                    subChapters.push([message.chapter[0],message.chapter[1],message.chapter[2],message.chapter[3],message.chapter[4],message.chapter[5]]);
                                    idx=0;

                                    for(var n = 0; n < subChapters.length; n++) {
                                        elem = subChapters[n];

                                        if (subChapters[n][5] ==message.chapter[5]){
                                            listsubchapters.push( createChapter({id: elem[0], title: elem[1], isChapter: elem[3] == 1, status: elem[4], url_title: elem[2], parent: elem[5]}));
                                            idx++;
                                        }
                                    }
                                    ...
                                    //var sublist =$('#item_'+message.chapter[0]+ " ul");
                                    var ch = getChapter(message.chapter[5]);
                                    $("#item_"+message.chapter[5]).replaceWith(makeChapterCompleteLine(message.chapter[5], ch.title, getStatusDescription(ch.status),listsubchapters));
                                    //  var sublist  = line.find('.subchapterslist');
                                    //agregar a la lista de subchapters?
                                    // makeSubChapterLine(message.chapter[0], message.chapter[1], getStatusDescription( message.chapter[4])).appendTo(sublist);
                                }else{
                                ...
                                toc.addItem(createChapter({id: message.chapter[0], title: message.chapter[1], isChapter: false, status: message.chapter[4], url_title: message.chapter[2]}));
                                var v = toc.getItemById(message.chapter[0]);
                                makeSectionLine(v.id, v.title, getStatusDescription(v.status)).appendTo("#chapterslist");
                            }
                            checkForEmptyTOC();
                        },
                        ...
                        "chapter_delete": function() {
                            ...
                            if (message.isSubChapter == 0){
                                var item = toc.getItemById(message.chapterID);
                                if(item){
                                    toc.delItemById(message.chapterID);
                                    toc.redraw();
                                }else{
                                ...
                                for(var n = 0; n < subChapters.length; n++) {
                                    elem = subChapters[n];
                                    if (subChapters[n][0] ==message.chapterID){

                                        // var parentID =subChapters[n][5];
                                        subChapters.splice(n,1);

                                        //get list of subchapters
                                        var csub = 0;
                                        listsubchaptersp = [];
                                        for(var m = 0; m < subChapters.length; m++) {
                                            elem = subChapters[m];

                                            if (subChapters[m][5] ==parentID){
                                                listsubchaptersp.push( createChapter({id: elem[0], title: elem[1], isChapter: elem[3] == 1, status: elem[4], url_title: elem[2], parent: elem[5]}));
                                                csub++;
                                            }
                                        }

                                        var ch = getChapter(parentID);
                                        if (csub>0){

                                            $("#item_"+parentID).replaceWith(makeChapterCompleteLine(parentID, ch.title, getStatusDescription(ch.status),listsubchaptersp));
                                        }

                                        else
                                            $("#item_"+parentID).replaceWith(makeChapterLine(parentID, ch.title, getStatusDescription(ch.status)));

                                        //  $("#item_"+message.chapterID).empty();
                                        break;
                                    }
                                }
                                // toc.redraw();
